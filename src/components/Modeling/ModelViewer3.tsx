...
export default function Modeling3({language}: Props) {
    const [buttonIndex, setButtonIndex] = useState<number>();
    const [cameraPosition, setCameraPosition] = useState<[number, number, number]>([0, 0, 3.2]);
    const [cameraTarget, setCameraTarget] = useState<[number, number, number]>([0, 0, 0]);
    useEffect(() => {
        switch (buttonIndex) {
            case 0:
                setCameraPosition([0, 0.5, 3.2]);
                ...
                {buttonIndex === 0 && (
                  <div className="features-01">
                      <div className="video-area" style={{aspectRatio: "441 / 249", width: "100%", height: "100%"}}>
                          <iframe
                            src="https://player.vimeo.com/video/1077166847?h=abcde12345&autoplay=1&loop=1&muted=1"
