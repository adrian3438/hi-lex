...
export default function Events({language, page}: Props) {
    ...
    const fetchNews = async (searchKeyword?: string) => {
        const response = await api.get(
            `/user/promotion/getContentsList.php?contentType=3&&userLang=${
                ...
                {newsList?.map((item: any, index: number) => (
                    <li key={index}>
                        <Link href={`/news/events/${item.ID}`}>
                            <Image
                                src={item.thumnailFile ? item.thumnailFile : "/images/@temp/events-sample.png"}
                                alt=""
                                width={511}
                                height={279}
                            />
