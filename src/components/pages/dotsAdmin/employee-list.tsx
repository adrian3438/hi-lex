...
export default function EmployeeListPage({
  ...
  async function listDelete(listID: string) {
    try {
      const formData = new FormData();
      formData.append('ID', listID);
      const response = await api.post('/admin/recruit/delRecruit.php', formData);
      if (response?.data?.result === true) {
        alert(response?.data?.resultMsg);
        getList();
      } else {
        alert('삭제 중 오류가 발견되었습니다.');
      }
    } catch (error) {
      console.log(error);
    }
                ...
                <tr key={index}>
                  ...
                  <td>
                    <div className="statUsBox">
                      <span className="gray" onClick={() => listDelete(list?.ID)}>삭제</span>
                    </div>
                  </td>
                </tr>
              ))}
