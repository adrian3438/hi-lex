...
setTimeout(function(){a.showChatBotBtnTips()},100);a.handleBtnDrag({$el:a.chatBotBtn[0],parent:a.parent,callback:a.beforeShow.bind(a)});a.chatBotBtn.on("keydown",function(e){if("Enter"===e.key||13===e.keyCode)e.preventDefault(),a.beforeShow(e)});d.jumpToEnd()})},setBtnSize:function(){var a=this,b=bookConfig.chatBotButtonIconHeight;this.chatBotBtn.css({"z-index":"98",position:"absolute",bottom:"80px",right:"50px","box-sizing":"border-box",height:b+"px",border:0,background:"transparent",borderRadius:"50%",
overflow:"hidden"});this.chatBotBtn.find("img").css({"box-sizing":"border-box",width:"100%",height:"100%",pointerEvents:"none"});var c=this.chatBotBtn.find("img")[0];c.complete?this.chatBotBtn.css({width:c.naturalWidth*b/c.naturalHeight+"px"}):(c.addEventListener("load",function(){a.chatBotBtn.css({width:c.naturalWidth*b/c.naturalHeight+"px"})}),c.onerror=function(){console.log("iconImg onerror")})},showChatBotBtnTips:function(){var a=this,b=this.chatBotBtn[0].getBoundingClientRect();this.parent.append(this.chatBotBtnTips);
this.chatBotBtnTips.fadeIn();this.isChatBotBtnTipsVisible=!0;this.chatBotBtnTips.css({display:"flex","box-sizing":"border-box",position:"absolute",bottom:window.innerHeight-b.bottom+"px",right:window.innerWidth-b.left+8+"px",cursor:"pointer"});this.chatBotBtnTips[0].addEventListener("click",function(c){c.target===a.chatBotBtnTips.find(".close-icon")[0]?(a.hidePopupTips(),BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"chatbot","close chatBot tips"])):a.beforeShow()});this.chatBotBtnTips.on("keydown",
function(c){if("Enter"===c.key||13===c.keyCode)c.preventDefault(),a.beforeShow(c)})},handleBtnDrag:function(a){function b(p){this.isDraging=!0;n=!1;m=e.getBoundingClientRect();p=p.center;h=p.x-m.x;k=p.y-m.y;$(e).css({cursor:"move"})}function c(p){if(this.isDraging){var q=Math.abs(p.center.y-(m.y+k));if(5<Math.abs(p.center.x-(m.x+h))||5<q)n=!0;var r=p.center;p=f.width();q=f.height();var u=e,t=$(u).outerWidth(),x=$(u).outerHeight();u=l.isChatBotBtnTipsVisible?r.x-h:r.x-t/2;r=l.isChatBotBtnTipsVisible?
r.y-k:r.y-x/2;u=Math.min(p-t,Math.max(0,u));r=Math.min(q-x,Math.max(0,r));newRight=Math.min(p-u-t,Math.max(p,u));newBottom=Math.min(q-r-x,Math.max(q,r));$(e).css({left:u+"px",top:r+"px",right:"initial",bottom:"initial"});l.chatBotBtnTips[0]&&(p=l.chatBotBtnTips[0].getBoundingClientRect(),l.chatBotBtnTips.css({left:u-p.width-8+"px",right:"initial",bottom:window.innerHeight-r-m.height+"px"}))}}function d(){this.isDraging=!1;$(e).css({cursor:"pointer"})}var e=a.$el,f=a.parent,g=a.callback;a=new Hammer(e);
a.add(new Hammer.Pan({threshold:0,pointers:0}));var h=0,k=0,m,l=this,n=!1;a.on("panstart panmove panend",function(p){p.preventDefault();switch(p.type){case "panstart":b(p);break;case "panmove":c(p);break;case "panend":d(p)}});a.on("tap",function(p){n||g&&g(p)})},hidePopupTips:function(){this.chatBotBtnTips&&(this.isChatBotBtnTipsVisible=!1,this.chatBotBtnTips.fadeOut(),this.setPopupTipsVisible("false"))},getPopupTipsVisible:function(){var a=getHashParams("chatbotVisible");if(a&&"true"==a)return!1;
try{var b=window.localStorage.getItem("isChatbotPopupTipsVisible");return b?JSON.parse(b):!0}catch(c){}},setPopupTipsVisible:function(a){try{window.localStorage.setItem("isChatbotPopupTipsVisible",a)}catch(b){}},handleBoxDrag:function(a){function b(m){this.isDraging=!0;var l=g.getBoundingClientRect();m=m.center;h=m.x-l.x;k=m.y-l.y;$(e).css({cursor:"move",width:"100%",height:"100%"})}function c(m){if(this.isDraging){var l=m.center;m=f.width();var n=f.height(),p=$(g).outerWidth(),q=$(g).outerHeight(),
r=l.x-h;l=l.y-k;r=Math.min(m-p,Math.max(0,r));l=Math.min(n-q,Math.max(0,l));$(g).css({bottom:"initial",right:"initial",left:r+"px",top:l+"px"})}}function d(){this.isDraging=!1;$(e).css({cursor:"pointer",width:"80%",height:"50px"})}var e=a.$ele,f=a.parent,g=a.$box;a=new Hammer(e);a.add(new Hammer.Pan({threshold:0,pointers:0}));var h=0,k=0;a.on("panstart panmove panend",function(m){m.preventDefault();switch(m.type){case "panstart":b(m);break;case "panmove":c(m);break;case "panend":d(m)}})},beforeShow:function(a){this.hidePopupTips();
BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"chatbot","chatbot button"]);this.showOrHide()},isRtl:function(){var a,b,c=/^(hebrew|arabic)$/.test(null==(a=selectedLanguage)?void 0:null==(b=a.language)?void 0:b.toLowerCase());return bookConfig.RightToLeft||c},initHtml:function(){this.container=$('<div class="chatbot-container"></div>');this.chatbotBox=$('<div id="chatbotBox" class="chatbot-box"></div>');this.chatbotDrag=$('<div id="draggableBox" class="chatbot-drag"></div>');this.chatbotResize=
$('<div id="resizableBox" class="chatbot-resize"></div>');this.resizeLeft=$('<div class="chatbot-resize-item chatbot-resize-left"></div>');this.resizeRight=$('<div class="chatbot-resize-item chatbot-resize-right"></div>');this.resizeTop=$('<div class="chatbot-resize-item chatbot-resize-top"></div>');this.resizeBottom=$('<div class="chatbot-resize-item chatbot-resize-bottom"></div>');this.resizeMask=$('<div class="chatbot-resize-mask"></div>');var a=_get(window,"website_domian_wc","https://fliphtml5.com"),
b=removeUrlParts(window.location.href);bookConfig.socialShareLink&&(b=bookConfig.socialShareLink);var c=getMetaTagValueByName("description"),d=getMetaTagValueByName("keywords");a+="/chatbot/chatbot.php";b=Metacharacter(b);d=Metacharacter(d);c=Metacharacter(c);var e=this.isRtl()?"1":"0",f,g,h=(null==(f=selectedLanguage)?void 0:null==(g=f.language)?void 0:g.toLowerCase())||"english";this.frame=$("<iframe id='chatbotIframe' allow=\"microphone\" class='chatbot-frame' src='"+(a+"?url="+b+"&desc="+c+"&keyword="+
d+"&isRtl="+e+"&lang="+h)+"' ></iframe>");this.container.append(this.chatbotBox);this.container.append(this.chatBotBtn);this.parent.append(this.container);this.chatbotBox.css({"box-sizing":"border-box","z-index":9999,position:"absolute",right:"120px",bottom:"50px",width:"421px",height:"740px",opacity:0,visibility:"hidden","border-radius":"12px"});this.chatbotResize.append(this.frame);this.chatbotResize.append(this.resizeLeft);this.chatbotResize.append(this.resizeRight);this.chatbotResize.append(this.resizeTop);
this.chatbotResize.append(this.resizeBottom);this.chatbotResize.append(this.resizeMask);this.chatbotBox.append(this.chatbotDrag);this.chatbotBox.append(this.chatbotResize);this.initResizeCss();this.chatbotDrag.css({width:"80%",left:"50%",transform:"translateX(-50%)"})},initEvent:function(){var a=this;window.addEventListener("message",function(b){var c=(null==b?void 0:b.data)||{},d=void 0===c.eventName?"":c.eventName;b=void 0===c.data?{}:c.data;if("chatbot"===(void 0===c.type?"":c.type).toLowerCase())switch(d){case "close":a.hide();
...
b)}))}});this.handleResize()},handleResize:function(){function a(l,n){function p(t){this.isResizing=!0;g=t.center.x;h=t.center.y;k=c.width();m=c.height();switch(n){case "right":c.css({right:"initial",left:t.center.x-k+"px"});break;case "left":c.css({right:"initial",left:t.center.x+"px"});break;case "top":c.css({bottom:"initial",top:t.center.y+"px"});break;case "bottom":c.css({bottom:"initial",top:t.center.y-m+"px"})}$(l).css({cursor:"move"});d.css({display:"block"});b.frame.css({"box-shadow":"none",
border:"none"})}function q(t){if(this.isResizing){var x=Math.min(t.center.x-g,e),v=Math.min(t.center.y-h,f);if(!(t.center.x>e||t.center.y>f||0>t.center.x||0>t.center.y))switch(n){case "right":c.css({width:k+x+"px"});break;case "left":c.css({left:t.center.x+"px",width:k-x+"px"});break;case "top":c.css({top:t.center.y+"px",height:m-v+"px"});break;case "bottom":c.css({height:m+v+"px"})}}}function r(){this.isResizing=!1;"left"!=n&&"right"!=n||$(l).css({cursor:"ew-resize"});"top"!=n&&"bottom"!=n||$(l).css({cursor:"ns-resize"});
d.css({display:"none"});b.frame.css({"box-shadow":"0 0 20px 0 rgba(0, 0, 0, 0.1)",border:"1px solid rgba(0,0,0,0.1)"})}var u=new Hammer(l);u.add(new Hammer.Pan({threshold:0,pointers:0}));u.on("panstart panmove panend",function(t){t.preventDefault();switch(t.type){case "panstart":p(t);break;case "panmove":q(t);break;case "panend":r(t)}})}document.querySelectorAll(".chatbot-resize-item").forEach(function(l){var n=l.classList[1].split("-")[2];a(l,n)});var b=this,c=this.chatbotBox,d=this.resizeMask,e=
this.parent.width(),f=this.parent.height(),g=0,h=0,k=0,m=0},initResizeCss:function(){this.resizeMask.css({display:"none",position:"absolute",left:0,top:0,right:0,bottom:0,"z-index":1E3});this.resizeLeft.css({position:"absolute",left:0,top:0,bottom:0,width:"5px",height:"100%",cursor:"ew-resize","z-index":1E3});this.resizeRight.css({position:"absolute",right:0,top:0,bottom:0,width:"5px",height:"100%",cursor:"ew-resize","z-index":1E3});this.resizeTop.css({position:"absolute",left:0,top:0,right:0,height:"5px",
width:"100%",cursor:"ns-resize","z-index":1E3});this.resizeBottom.css({position:"absolute",left:0,bottom:0,right:0,width:"100%",height:"5px",cursor:"ns-resize","z-index":1E3})},showOrHide:function(){this.visible?this.hide():this.show()},show:function(){var a=this;this.visible||(this.visible=!0,this.chatbotBox.css({opacity:1,visibility:"visible"}),this.handleBoxDrag({$ele:this.chatbotDrag[0],parent:this.parent,$box:this.chatbotBox[0]}),this.postMessage({messageType:"show"}),setTimeout(function(){a.focusIframe()},
100),FormManager.register(this))},hide:function(){var a=this;this.visible&&(this.visible=!1,this.chatbotBox.css({opacity:0,visibility:"hidden"}),this.postMessage({messageType:"hide"}),this.blurIframe(),setTimeout(function(){a.chatBotBtn.focus()},100),FormManager.unregister(this))},postMessage:function(a){var b=document.querySelector("#chatbotIframe");b&&b.contentWindow.postMessage(a,"*")},focusIframe:function(){try{this.frame[0].focus()}catch(a){console.log("\u8bbe\u7f6eiframe\u7126\u70b9\u5931\u8d25:",
a)}},blurIframe:function(){try{this.frame[0].blur()}catch(a){console.log("\u79fb\u9664iframe\u7126\u70b9\u5931\u8d25:",a)}},handleIframeReady:function(){var a=this;this.visible&&setTimeout(function(){a.focusIframe()},100)},onResize:function(a,b){a=this.chatbotBox.height();this.chatbotBox.css({height:Math.min(b-80,a)+"px"})},changeLanguage:function(a){this.chatBotBtnTips&&this.chatBotBtnTips.find("span").text(getLanguage("chatbotBtnTips","Hi~ Ask about the book, or anything!"));this.postMessage({eventName:"changeLang",
type:"chatBot",data:{lang:a}})}});
